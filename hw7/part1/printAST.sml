...
fun expressionString (EXP_ID s) = s
  ...
  | expressionString (EXP_ANON {params, body}) =
   ...
   ")"
  | expressionString (EXP_PROPERTY {lhs, rhs}) = 
    (expressionString lhs) ^ ": " ^ (expressionString rhs)
  | expressionString (EXP_NEW {newExp}) = 
    "(new " ^ (expressionString newExp) ^ ")"
  | expressionString (EXP_THIS) = "this"
  | expressionString (EXP_DOT {lhs, rhs}) = 
    "(" ^ (expressionString lhs) ^ "." ^ (expressionString rhs) ^ ")"
  | expressionString (EXP_OBJ {objList}) = 
    "{\n" ^ (propertyString objList) ^ "}"
and propertyString [] = ""
  | propertyString (p::ps) = 
    (expressionString p) ^ 
      (String.concat (List.map (fn prop => (", " ^ (expressionString prop))) ps))
and argumentsString [] = ""
