...
and parseCallExpression fstr tk =
   let
      ...
      val (optArgs, tk2) = parseOptionalArguments fstr tk1 func
   in
      parseOptAccArgs  fstr tk2 optArgs
   end
and parseMemberExpression fstr tk = 
  case tk of
    (TK_NEW) => (
      let 
        val (member, tk1) = parseMemberExpression fstr (nextToken fstr)
        val (expression, tk2) = parseArguments fstr tk1 member
      in
        parseOptAccess fstr tk2 (EXP_NEW {newExp = expression})
      end
    )
    | _ => (parsePrimaryExpression fstr tk)
and parseOpAccHelper fstr tk (lhsExp:expression) =
        if (tk = TK_DOT) then
            let 
            ...
            in
              (case tk1 of
                (TK_ID _) => (
                  let val (rhsExp, tk2) = parsePrimaryExpression fstr tk1
                  in parseOpAccHelper fstr tk2 (EXP_DOT {lhs=lhsExp, rhs=rhsExp})
                  end
                )
            ...
            (lhsExp, tk)
and parseOptAccess fstr (tk as TK_DOT) memberExp = parseOpAccHelper fstr tk memberExp
  | parseOptAccess fstr tk exp = (exp, tk)
and parseOptAccArgs fstr tk memberExp = 
    case tk of
        (TK_DOT) => (
          let 
            val (expression, tk1) = parseOptAccess fstr tk memberExp
          in 
            (parseOptAccArgs  fstr tk1 expression)
          end
        )
      | (TK_LPAREN) => (
          let 
            val (expression, tk1) = parseOptionalArguments fstr tk memberExp
          in 
            (parseOptAccArgs  fstr tk1 expression)
          end
      )
      | _ => (memberExp, tk) (*memberExp here is just a normal expression*)
and parsePrimaryExpression fstr (tk as TK_LPAREN) =
  ...
  | parsePrimaryExpression fstr (tk as TK_LBRACE) = parseObjLit fstr tk
  | parsePrimaryExpression fstr (tk as TK_THIS) = (EXP_THIS, (nextToken fstr))     
  | parsePrimaryExpression fstr tk = err_expect "value" (tkString tk)   
and parseObjLit fstr (tk as TK_LBRACE) =   
  let 
    val tk1 = nextToken fstr   
  in
    ( case tk1 of
      (TK_ID _) => (
        let 
          val (lst, tk2) = parseCommaSeparatedSequence parsePropAssign fstr tk1    
        in 
          (EXP_OBJ {objList=lst}, (match_tk fstr tk2 TK_RBRACE))    
        end
      )
      | _ => (EXP_OBJ {objList=[]}, (match_tk fstr tk1 TK_RBRACE)) 
    )
  end    
  | parseObjLit fstr tk = err_expect "{" (tkString tk)   
and parsePropAssign fstr (tk as TK_ID _) =    
  let    
      val (lhsExp, tk1) = parsePrimaryExpression fstr tk   
      val (rhsExp, tk2) = parseAssignmentExpression fstr (match_tk fstr tk1 TK_COLON)    
  in 
    (EXP_PROPERTY {lhs=lhsExp, rhs=rhsExp}, tk2)    
  end    
  | parsePropAssign fstr tk = err_expect "identifier" (tkString tk)   
and parseArguments fstr (tk as TK_LPAREN) func =    
   parseOptionalArguments fstr tk func    
  | parseArguments _ tk _ = err_expect "(" (tkString tk)
and parseOptionalArguments fstr (tk as TK_LPAREN) func =
   let
      ...
      val tk3 = match_tk fstr tk2 TK_RPAREN;
   in
      parseOptionalArguments fstr tk3 (EXP_CALL {func=func, args=args})
   end
